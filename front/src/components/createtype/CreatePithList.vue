<template>
  ...
  <div class="town-list" v-else>
    <div class="container">
      <h2 class="town-form__headline">Створення назви залишків</h2>
      <form class="town-form__create" @submit.prevent="createProductName">
        <input type="text" class="town-form__create--input input" placeholder="Назва залишків" v-model="name"
      ...
      <div class="town-list__box-body" v-for="productName in productNames" :key="productName.id">
        <div class="town-list__item-body">
          <span class="town-list__item-body--responsive">Назва залишків: </span>{{productName.name}}
        </div>
...
<script>
import ModalEditPithNameCreate from '../modals/ModalEditPithNameCreate'
...
export default {
  ...
  methods: {
    async getProductNames() {
      try {
        const result = await axios.get(`${process.env.VUE_APP_BASE_URL}/api/pithname`, {
    ...
    async createProductName() {
      ...
      await this.$store.dispatch('createPithName', formData)
      this.name = ''
      this.getProductNames()
    },
    async deleteProductName(id) {
      try  {
        ...
        if(agree === "1111") {
          await axios.delete(`${process.env.VUE_APP_BASE_URL}/api/pithname/${id}`, {
            headers: {Authorization: `Bearer ${localStorage.getItem('token')}`}
          })
          this.getProductNames()
        }
