    ...
    <div class="cash-table" v-else>
        <div class="container">
            ...
            <div>
              ...
              <button class="button create-order__button" @click="getCashs(startBackDate, endBackDate)">Вивести інформацію</button>
            </div>
            ...
            <div class="cash-table__box-body" v-for="(cash, index) in filteredCash" :key="cash.id"
                :class="{'check': cash.product_name === 'Перевірка'}"
            >
                ...
                <div class="cash-table__item-body" v-if="user.role_id == 1">
                    <span class="cash-table__item-body--responsive">Дата створення: </span>{{dateFormatForCreateItem(cash.data_create)}}
                </div>
                <div class="cash-table__item-body" @click="showModalComments(cash.id)">
                  <span class="cash-table__item-body--responsive">Коментар: </span>
                  <span v-if="cash.comment" class="comment-is">Є коментар</span>
                </div>
...
export default {
    ...
    methods: {
        async getCashs(start, end) {
          ...
          const result = await axios.get(`${process.env.VUE_APP_BASE_URL}/api/cashmoney?start=${start || lastMonthDay}&end=${end || currentDay}`, {
