<template>
  <div class="modal-create">
    ...
    <div class="modal-create__box">
      <h2 class="modal-create__headline">Редагування назви залишків</h2>
      <form class="modal-create__form" @submit.prevent="changeProductName">
        <p class="modal-create__form--text">Введіть нову назву залишка в полі нижче</p>
        <input type="text" class="modal-create__form--input input" placeholder="Назва залишка" v-model="name"
               :class="{invalid: ($v.name.$dirty && !$v.name.required)}"
        >
        <small
            class="helper-text__invalid"
            v-if="$v.name.$dirty && !$v.name.required"
        >Поле назви залишків не повинно бути пустим!</small>
...
export default {
  ...
  methods: {
    async changeProductName() {
      ...
      try  {
        await this.$store.dispatch('updatePithName', formData)
        this.RemoveModal()
      } catch (e) {
