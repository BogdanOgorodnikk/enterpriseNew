<template>
    <div class="modal-create">
        ...
        <div class="modal-create__box">
            <h2 class="modal-create__headline">Редагування назви района</h2>
            <form class="modal-create__form" @submit.prevent="changeAreaName">
                <p class="modal-create__form--text">Введіть нову назву района в полі нижче</p>
                <input type="text" class="modal-create__form--input input" placeholder="Назва района" v-model="name"
...
export default {
    ...
    methods: {
        async changeAreaName() {
            ...
            try  {
                await this.$store.dispatch('updateAreaName', formData)
                this.RemoveModal()
            } catch (e) {
