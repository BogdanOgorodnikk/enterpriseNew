<template>
  <div class="modal-create">
    ...
    <div class="modal-create__box">
      <h2 class="modal-create__headline">Редагування назви форми</h2>
      <form class="modal-create__form" @submit.prevent="changeNoteName">
        <p class="modal-create__form--text">Введіть нову назву форми в полі нижче</p>
        <input type="text" class="modal-create__form--input input" placeholder="Назва форми" v-model="name"
               :class="{invalid: ($v.name.$dirty && !$v.name.required)}"
        >
        <small
            class="helper-text__invalid"
            v-if="$v.name.$dirty && !$v.name.required"
        >Поле назви форми не повинно бути пустим!</small>
...
export default {
  ...
  methods: {
    async changeNoteName() {
      ...
      try  {
        await this.$store.dispatch('updateNoteName', formData)
        this.RemoveModal()
      } catch (e) {
