<template>
    <div class="modal-create">
        ...
        <div class="modal-create__box">
            <h2 class="modal-create__headline">Редагування назви області</h2>
            <form class="modal-create__form" @submit.prevent="changeRegionName">
                <p class="modal-create__form--text">Введіть нову назву області в полі нижче</p>
                <input type="text" class="modal-create__form--input input" placeholder="Назва області" v-model="name"
...
export default {
    ...
    methods: {
        async changeRegionName() {
            ...
            try  {
                await this.$store.dispatch('updateRegionName', formData)
                this.RemoveModal()
            } catch (e) {
