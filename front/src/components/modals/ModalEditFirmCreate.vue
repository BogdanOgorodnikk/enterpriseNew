<template>
    <div class="modal-create">
        ...
        <div class="modal-create__box">
            <h2 class="modal-create__headline">Редагування назви фірми</h2>
            <form class="modal-create__form" @submit.prevent="changeFirmtName">
                <p class="modal-create__form--text">Введіть нову назву фірми в полі нижче</p>
                <input type="text" class="modal-create__form--input input" placeholder="Назва фірми" v-model="name"
                    :class="{invalid: ($v.name.$dirty && !$v.name.required)}"
                > 
                <small 
                    class="helper-text__invalid"
                    v-if="$v.name.$dirty && !$v.name.required"
                >Поле назви фірми не повинно бути пустим!</small>
...
export default {
    name: "ModalEditFirmCreate",
    props: ['showModalEdit', 'id', 'name'],
    validations: {
        name: {required}
    },
    methods: {
        async changeFirmtName() {
            if(this.$v.$invalid) {
                this.$v.$touch()
                return
            }
            ...
            try  {
                await this.$store.dispatch('updateFirmName', formData)
                this.RemoveModal()
            } catch (e) {
