<template>
    <div class="modal-create">
        ...
        <div class="modal-create__box">
            <h2 class="modal-create__headline">Редагування оптової ціни</h2>
            <form class="modal-create__form" @submit.prevent="changeOptPrice">
                <p class="modal-create__form--text">Введіть нову оптову ціну в полі нижче</p>
                <input type="text" class="modal-create__form--input input" placeholder="Оптова ціна" v-model="number"
                    :class="{invalid: ($v.number.$dirty && !$v.number.required)}"
                >
                <small 
                    class="helper-text__invalid"
                    v-if="$v.number.$dirty && !$v.number.required"
                >Поле оптової ціни не повинно бути пустим!</small>

                <select class="create-opt-price-select" v-model="firmEditInfo">
...
export default {
    name: "ModalEditOptCreate",
    props: ['showModalEdit', 'id', 'number', 'firms', 'firmInfo'],
    data: () => ({
      firmEditInfo: {}
    }),
    validations: {
    ...
    mounted() {
      this.firmEditInfo = this.firmInfo
    },


  methods: {
        async changeOptPrice() {
            if(this.$v.$invalid) {
                this.$v.$touch()
                return
            }
            const formData = {
                id: this.id, 
                number: this.number,
                firm_name: this.firmEditInfo.name,
                firm_id: this.firmEditInfo.id
            }
            try  {
                await this.$store.dispatch('updateOptPrice', formData)
                this.RemoveModal()
            } catch (e) {
