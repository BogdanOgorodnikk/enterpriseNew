...
export default {
  ...
  methods: {
    async getComments() {
      try {
        const result = await axios.get(`${process.env.VUE_APP_BASE_URL}/api/preparecomment/${this.id}`, {
    ...
    async createComment() {
      ...
      try  {
        await this.$store.dispatch('createPrepareOrderComment', formData)
        this.description = ''
      } catch (e) {
        alert('Відбулася помилка')
      }
    ...
    async onClickUpdateComment() {
      ...
      try  {
        await this.$store.dispatch('updatePrepareOrderComment', formData)
        this.description = ''
    ...
    async onClickDeleteComment(id) {
      try  {
        ...
        if(agree === "1111") {
          await axios.delete(`${process.env.VUE_APP_BASE_URL}/api/preparecomment/${id}`, {
            headers: {Authorization: `Bearer ${localStorage.getItem('token')}`}
          })
          this.getComments()
        }
